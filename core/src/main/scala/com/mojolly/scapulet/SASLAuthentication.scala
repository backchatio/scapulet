...
import com.mojolly.scapulet.Exceptions.SASLAuthenticationFailed
import com.mojolly.scapulet.Scapulet.ScapuletConnection
...
object SASLAuthentication {
  ...
  object Success {
    def apply(data: String) = <success xmlns={SASL_NS}>{data}</success>
    def unapply(node: Node) = node match {
  ...
  object Challenge {
    def apply(data: String) = <challenge xmlns={SASL_NS}>{data}</challenge>
    def unapply(node: Node) = node match {
  ...
  object Response {
    def apply(data: String) = {
      <response xmlns={SASL_NS}>{data}</response>
    }
  }
  ...
  trait SASLMechanism extends CallbackHandler {
    ...
    protected def authenticate() {
      ...
      connection.write(<auth mechanism={name} xmlns={SASL_NS}>{authText}</auth>)
    }
    ...
    def onServerChallenge(challenge: String) {
      ...
      } else { saslClient.evaluateChallenge(new Array[Byte](0)) }
    ...
    def handle(callbacks: Array[Callback]) {
      callbacks foreach {
        case cb: NameCallback => { cb.setName(_userName) }
        case cb: PasswordCallback => {
          cb.setPassword(_password)
        }
        case cb: RealmCallback => { cb.setText(_hostName) }
